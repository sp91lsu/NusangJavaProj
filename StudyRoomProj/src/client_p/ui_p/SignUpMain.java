...
public class SignUpMain extends JFrame implements Receivable, MouseListener{
	...
	public void receive(PacketBase packet) {
		...
		if (packet.getClass() == ScSignUpAck.class) {//·Î±×ÀÎ È®ÀÎ
			...
			if ((ack.eResult == EResult.SUCCESS) && !ack.is_hp) {//id¼º°ø
				...
				jl2 = new JLabel("»ç¿ë °¡´ÉÇÑ ID ÀÔ´Ï´Ù.");
			...
			if ((ack.eResult == EResult.SUCCESS) && ack.is_hp) {//ÇÚµåÆù¼º°ø
				hpDialog = new JDialog();
				hpDialog.setBounds(50, 50, 150, 150);
				hpDialog.getContentPane().setLayout(new GridLayout(2, 1));
				hpLabel = new JLabel("»ç¿ë °¡´ÉÇÑ ÇÚµåÆù ÀÔ´Ï´Ù.");
				hpButton = new JButton("È®ÀÎ");
				...
				jd2.setBounds(50, 50, 150, 150);
				jd2.getContentPane().setLayout(new GridLayout(2, 1));
				jl2 = new JLabel("Áßº¹µÈ ID ÀÔ´Ï´Ù.");
				jb2 = new JButton("È®ÀÎ");
				jb2.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						JButton bbb = (JButton) e.getSource();
				...
				jd2.setBounds(50, 50, 150, 150);
				jd2.getContentPane().setLayout(new GridLayout(2, 1));
				jl2 = new JLabel("Áßº¹µÈ ÇÚµåÆù ÀÔ´Ï´Ù.");
				jb2 = new JButton("È®ÀÎ");
				jb2.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						JButton bbb = (JButton) e.getSource();
	...
	void phChk()
	{
		...
		if(phoneNum.matches(phoneChk))
		{
			...
			JLabel chkLb = new JLabel("Á¤È®ÇÑ ÇÚµåÆù Çü½ÄÀ» ÀÔ·ÂÇÏ¼¼¿ä");
			chkph.getContentPane().add(chkLb);
			JButton hpchkBt = new JButton("È®ÀÎ");
