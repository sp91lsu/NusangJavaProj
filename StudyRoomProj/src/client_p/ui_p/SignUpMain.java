...
public class SignUpMain extends JDialog implements Receivable, MouseListener, ActionListener, KeyListener {
	...
	public SignUpMain() {
		...
		JLabel titleLabel = new JLabel("È¸¿ø°¡ÀÔ");
	...
	public void receive(PacketBase packet) {
		...
		if (packet.getClass() == ScSignUpAck.class) {
			...
			if (ack.eResult == EResult.SUCCESS) {// È¸¿ø°¡ÀÔ ¼º°ø
				...
				AlreadyUsePop pop = new AlreadyUsePop("Áßº¹µÈ IDÀÔ´Ï´Ù");
			} else if (ack.eResult == EResult.DUPLICATEED_PHONE) {
				AlreadyUsePop pop = new AlreadyUsePop("Áßº¹µÈ ÈÞ´ëÆù¹øÈ£ ÀÔ´Ï´Ù");
			} else {
	...
	void check() {
		...
		try {
			if (name.matches(korean)) {
				...
				label_1.setText("ÇÑ±ÛÀÌ ¾Æ´Õ´Ï´Ù.");
				AlreadyUsePop pop = new AlreadyUsePop("Á¤È®ÇÑ Á¤º¸¸¦  ÀÔ·ÂÇÏ¼¼¿ä");
				return;
			}
			if (id.matches(eng) && id.matches(num)) {
				label_2.setText("ÀÔ·Â È®ÀÎ");
			} else {
				label_2.setText("ID Çü½Ä ¿À·ù");
				AlreadyUsePop pop = new AlreadyUsePop("Á¤È®ÇÑ Á¤º¸¸¦  ÀÔ·ÂÇÏ¼¼¿ä");
				return;
			}
			if (pass.matches(eng) && pass.matches(num)) {
				...
				label_3.setText("ºñ¹Ð¹øÈ£ Çü½Ä ¿À·ù");
				AlreadyUsePop pop = new AlreadyUsePop("Á¤È®ÇÑ Á¤º¸¸¦  ÀÔ·ÂÇÏ¼¼¿ä");
				return;
			}
			if (phoneNum.matches(phoneChk)) {
				...
				label_5.setText("ÈÞ´ëÆù ¹øÈ£ Çü½Ä ¿À·ù");
				AlreadyUsePop pop = new AlreadyUsePop("Á¤È®ÇÑ Á¤º¸¸¦  ÀÔ·ÂÇÏ¼¼¿ä");
				return;
			}
